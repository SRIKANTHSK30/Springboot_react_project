...
function Srii() {
    ...
    const handleClick =()=>{
      fetch("http://localhost:8080/h/get/id="+delid).then(res=> res.json()).then(res => SetUser(res));
      console.log(user);
      if(user==null) alert("Please enter the Valid Id");
      else{
      fetch("http://localhost:8080/h/del?id="+delid, {
        method: "DELETE",
        headers: {"Accept":"application/json", "Content-Type":"application/json",}
    }).then(res => alert("ID Deleted"));
  }
    }
...
<input type="text" id="name" name="name" placeholder="Type your Age.." autocomplete="off" required 
value={age}
onChange={(event) =>
  {
